...
class ChessBoardDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = fen_to_board(row['fen'])  # shape (8,8)
...
def main():
    print("🚀 Starting Optimized Chess Board Training with Diverse Dataset")
    print("=" * 70)
    ...
    # Dataset paths for diverse dataset
    csv_path = 'diverse_dataset/labels.csv'
    images_dir = 'diverse_dataset/images'
    
    # Check if diverse dataset exists
    if not os.path.exists(csv_path):
        print(f"❌ Diverse dataset not found at {csv_path}")
        print("Please run generate_diverse_chess_dataset.py first")
        return
    ...
    # Enhanced data transforms for diverse dataset
    train_transform = transforms.Compose([
        transforms.Resize((256, 256)),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomRotation(degrees=10),  # Slightly more rotation
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2, hue=0.1),  # More color variation
        transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1)),  # Slight affine transforms
        transforms.ToTensor(),
    ...
    train_dataset = ChessBoardDataset(csv_path, images_dir, split='train', transform=train_transform)
    val_dataset = ChessBoardDataset(csv_path, images_dir, split='val', transform=val_transform)
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            }, 'best_chessboard_net_diverse.pth')
    ...
    print(f"💾 Model saved as: best_chessboard_net_diverse.pth")
